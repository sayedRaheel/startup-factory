...
mkdir -p mergeguard tests

# 5. Create core files and populate them
...
cat << 'EOF' > mergeguard/git_utils.py
...
cat << 'EOF' > mergeguard/llm_client.py
import os
...
cat << 'EOF' > mergeguard/analyzer.py
...
cat << 'EOF' > README.md
# MergeGuard
Defensive CLI tool to filter out unmergeable AI slop from PRs.
...
echo "MergeGuard project successfully created and populated!"
```
